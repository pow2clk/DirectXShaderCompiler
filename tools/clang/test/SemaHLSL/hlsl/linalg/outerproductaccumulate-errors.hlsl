...
export void Test4(vector<half, 128> Input1, vector<half, 64> Input2) {
  ...
  // expected-note@dx/linalg.h:170{{candidate template ignored: could not match 0 against 1}}
...
export void Test4(vector<half, 64> Input1, vector<half, 64> Input2) {
  ...
  // expected-error@+3{{no matching function for call to 'OuterProductAccumulate'}}
  // expected-note@dx/linalg.h:170{{candidate template ignored: deduced conflicting types for parameter 'ElTy' ('int' vs. 'unsigned int')}}
